...
class AnalysisFormatter:
    _option_to_heading_map = {
        ...
        "НОС_НАКЛОН": {
            ...
            "Вздернутый": "ВЗДЕРНУТЫЙ КОНЧИК НОСА",
        ...
        "ДОП_СКУЛЫ": {
            ...
            "В перед": "ВЫРАЖЕННЫЕ СКУЛЫ ВПЕРЁД",
        },
        "ДОП_УШИ": {
            "Большие": "БОЛЬШИЕ УШИ",
    ...
    def normalize_heading(heading: str) -> str:
        """Нормализует заголовок для сравнения: приводит к верхнему регистру и удаляет лишние пробелы"""
        return ' '.join(heading.upper().split())
    ...
    def parse_raw_text_file(file_content: str) -> dict[str, str]:
        ...
        for category_map in AnalysisFormatter._option_to_heading_map.values():
            for original_heading in category_map.values():
                normalized = AnalysisFormatter.normalize_heading(original_heading)
                normalized_heading_to_original_map[normalized] = original_heading
        ...
        for line in lines:
            ...
            # Check if this line is a heading by comparing with normalized versions
            normalized_line = AnalysisFormatter.normalize_heading(line)
            if normalized_line in normalized_heading_to_original_map:
                # If we were processing a previous heading, save it
                if current_heading and current_description_lines:
                    data[current_heading] = '\n'.join(current_description_lines).strip()

                # Start new heading
                current_heading = normalized_heading_to_original_map[normalized_line]
                current_description_lines = []
            else:
        ...
        # Don't forget to save the last heading
        if current_heading and current_description_lines:
            data[current_heading] = '\n'.join(current_description_lines).strip()
